...
let copyPicsAndResources() =
    try
      CopyDir (outDir </> "images") (slidesDir </> "images") (fun f -> true)
      CopyDir (outDir </> "resources") (slidesDir </> "resources") (fun f -> true)
    with
    | exn -> traceImportant <| sprintf "Could not copy resource: %s" exn.Message

let generateFor (file:FileInfo) = 
    try
        copyPicsAndResources()
